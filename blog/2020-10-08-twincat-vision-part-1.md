...
tags: ['beckhoff', 'twincat', 'vision', 'function', 'struct', 'dry']
---

![background](./images/twincat-vision.jpg)

> First steps into Beckhoff's newly released TC3 Vision. Setting up basic metrology using the provided functions and simplifying them using custom structs and functions.
...
Having a seperate smart camera requires yet another engineering software tool to learn and use and have available and the added complexity of interfacing yet another third party device. Bringing the engineering setup into the same tool as the rest of the PLC development would streamline things.

The TwinCAT vision is API based versus having a GUI. The provided API has many filter functions (Gaussian, Laplace, Scharr, Sobel, Bilateral, etc) and can be quite powerful if your application needs it and you know your image processing. They do include basic measurement functions that should be sufficient for most basic metrology needs. These functions are `LocatedEdge`, `LocateEllipse`, `LocationCircularEdge`, `MeasureEdgeDistance`, `MeasureAngleBetweenEdges`, `ClosestPointBF`. They will require some basic parameterizaing, and doing this will likely take a little longer than having a GUI for point and click definitions.
...
### Background
Let's start with the final image with all found edges displayed. The red O is the start of the search line and the red X the end. The green lines are the best fit line while the blue X are the matched points. The original is 2500x2000px hence why the small markups.
...
VAR
	...
	// parameters
	d209Start : TcVnPoint2_REAL := [80, 190];
	d209Stop : TcVnPoint2_REAL := [78, 440];
	d222Start : TcVnPoint2_REAL := [183, 210];
	d222Stop : TcVnPoint2_REAL := [180, 420];
	d201Start : TcVnPoint2_REAL := [15, 312];
	d201Stop : TcVnPoint2_REAL := [165, 312];
	...
	nSubpixIter : UDINT := 50;
	eAlgorithm : ETcVnEdgeDetectionAlgorithm := TCVN_EDA_INTERPOLATION;
	
	nSearchLines201 : UDINT := 71;
	fSearchLineDist201 : REAL := 3;
	nMaxThickness201 : UDINT := 25;
	nSearchLines222 : UDINT := 23;
...
### Make it DRY
One unfortunate detail about using such specific functions with many parameters and since Structured Text does not support overloading functions or default paramters it makes all these function calls very verbose.

We can rectify this by using structs with default values and pass them to helper functions by reference. First define a new structure that will hold all potential function parameters.
...
And now the helper function to call the Beckhoff function:
```typescript
...
Finally define a function to draw the results:
```typescript
...
Now if we use these new blocks the previous code can be written as such. I personally much prefer this as it becomes much easier to read as it uses defaults for most parameters. They are there if you need them though. A measurement definition is now as simple as initilizing the struct with at a minimum the required values and any optional ones.
```typescript
VAR
	hr : HRESULT;
	hrFunc : HRESULT;
	...
	dimDist : ARRAY [0..2] OF ST_DimBase := [
		(name:='d201', pFrom:=[15, 312], pTo1:=[165, 312], lines:=71, lineDist:=3),
		(name:='d209', pFrom:=[80, 190], pTo1:=[78, 440], lines:=31, lineDist:=3),
		(name:='d222', pFrom:=[183, 210], pTo1:=[180, 420], lines:=23, lineDist:=2)
	];
END_VAR
...
This two points can be fixed by adding a function block to obscure out those inner workings.
...
Now in your main program you'd only need this as a base. Obviously some of the functionality is missing (start, stop, done, reset, etc) from the function block, but this FB can run as is and will trigger once upon start.
